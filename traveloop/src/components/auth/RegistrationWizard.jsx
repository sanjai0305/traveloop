...
const RegistrationWizard = ({ initialStep = 1 }) => {
  ...
  }, [formData, isResendDisabled, loading, resendCount]);
  ...
  const handleVerifyOtp = useCallback(
    async (e) => {
      ...
      try {
        ...
        setErrors({ general: err.message || "Invalid verification code." });
      } finally {
        setLoading(false);
      }
    },
  ...
  const handleCreateAccount = useCallback(
    ...
    [formData, validateStep3, login, navigate]
  );
  ...
  const renderStep1 = useMemo(
    ...
    [formData, errors, loading, handleFormChange, handleContinueToOtp]
  );
    ...
    () => (
      <form onSubmit={handleVerifyOtp} className="w-full max-w-md mx-auto space-y-4">
        ...
        <p className="text-xs text-slate-400 dark:text-slate-500 text-center px-4">
          Check spam/promotions folder if you don't see the code.
        </p>
      </form>
    ),
    [
      ...
      <form onSubmit={handleCreateAccount} className="space-y-4">
          ...
          {formData.password && (
            <div className="mt-1.5 space-y-1 animate-fade-in">
              <div className="flex justify-between items-center text-xs">
                <span className="text-slate-400 dark:text-slate-500 font-medium">Password Strength:</span>
        ...
        <div className="flex justify-center">
          <button
            ...
            disabled={loading}
            className="text-xs text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-400 transition-colors flex items-center gap-1.5 active:scale-95"
          >
            <ArrowLeft size={14} />
  ...
  const stepSubtitles = {
    1: "Join Traveloop and start planning your adventures around the world.",
    2: `We sent a 6-digit code to ${maskedEmail}`,
