    ...
    <MainLayout>
      <div className="px-4 pt-4 pb-6">
        ...
        {(!id || activeTab === "discover") && (
          <div>
            {/* Empty state — prompt */}
            {!discoverSearch && discoverResults.length === 0 && !discoverLoading && (
              <motion.div
              ...
              >
                ...
                <p className="text-xs text-slate-400 dark:text-slate-500 mb-5 max-w-[260px]">
                  Type Salem, Tokyo, Paris, Bali or any city — AI will instantly discover top attractions.
                </p>

                {/* Quick suggestions */}
                <div className="flex flex-wrap justify-center gap-2 mb-2">
                  {["Salem", "Tokyo", "Paris", "Bali", "Dubai", "Goa"].map(s => (
                    <motion.button
                      key={s}
                      whileTap={{ scale: 0.92 }}
                      onClick={() => { setDiscoverSearch(s); setInlineSearch(s); clearTimeout(debounceRef.current); fetchDiscover(s); }}
                      className="px-3.5 py-2 rounded-full text-xs font-bold border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:border-teal-300 transition-colors"
                    >
                      {s}
                    </motion.button>
                  ))}
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[13px] font-extrabold text-slate-800 dark:text-slate-100">
                        Near <span className="text-teal-600">{discoverQuery}</span>
                      </span>
                    </div>
                    <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-teal-50 dark:bg-teal-900/20 border border-teal-100 dark:border-teal-800">
                      ...
                      <span className="text-[9px] font-bold text-teal-600">{discoverResults.length} found</span>
                    </div>
                  </div>
            ...
            {/* No results */}
            {!discoverLoading && discoverSearch && discoverResults.length === 0 && discoverQuery && (
              <motion.div
              ...
              >
                ...
                <p className="text-xs text-slate-400">Try a different city or country name</p>
              </motion.div>
            )}
          </div>
        )}
